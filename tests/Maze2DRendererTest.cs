...
namespace Nour.Play {
    ...
    public class Maze2DRendererTest {
        ...
        [Test]
        public void GetCellTypes_IsValid() {
            var maze = Maze2D.Parse("4x4;0:1,4;1:2,5;2:3;3:7;4:5,8;8:12;12:13;13:14;14:10;10:11");
            // Console.WriteLine(maze.ToString());
            // ╔═══════════╗
            // ║0x3 1x3 2x3║3x3
            ...
            //           WTWWTTWTWWTTW
            //           001 1 223 3 4
            var mazeToMap = new Maze2DRenderer(maze, new MazeToMapOptions(
                trailXWidths: new int[] { 1, 2, 1, 2 },
                trailYHeights: new int[] { 2, 1, 2, 1 },
                wallXWidths: new int[] { 1, 2, 1, 2, 1 },
                wallYHeights: new int[] { 2, 1, 2, 1, 2 }));

            var cellTypes = mazeToMap.GetCellTypes(maze.Cells[new Vector(0, 0).ToIndex(maze.Size.X)]);

            Assert.That(cellTypes.nw, Is.EqualTo(MapCellType.Wall), "NW");
            Assert.That(cellTypes.n, Is.EqualTo(MapCellType.Trail), "N");
            Assert.That(cellTypes.ne, Is.EqualTo(MapCellType.Trail), "NE");
            Assert.That(cellTypes.w, Is.EqualTo(MapCellType.Wall), "W");
            Assert.That(cellTypes.center, Is.EqualTo(MapCellType.Trail), "Center");
            Assert.That(cellTypes.e, Is.EqualTo(MapCellType.Trail), "E");
            Assert.That(cellTypes.sw, Is.EqualTo(MapCellType.Wall), "SW");
            Assert.That(cellTypes.s, Is.EqualTo(MapCellType.Wall), "S");
            Assert.That(cellTypes.se, Is.EqualTo(MapCellType.Wall), "SE");
        }

        [Test]
        public void GetCellTypes_Edge() {
            var maze = Maze2D.Parse("4x4;0:1,4;1:2,5;2:3;3:7;4:5,8;8:12;12:13;13:14;14:10;10:11");
            var mazeToMap = new Maze2DRenderer(maze, new MazeToMapOptions(
                trailXWidths: new int[] { 1, 2, 1, 2 },
                trailYHeights: new int[] { 2, 1, 2, 1 },
            ...
            var cellTypes = mazeToMap.GetCellTypes(maze.Cells[new Vector(1, 0).ToIndex(maze.Size.X)]);

            // GetCellTypes returns proposed map cell types based on the current
            // maze cell. It only looks at X++ and Y++, so in theory we should
            // not check x<X (w) and y<Y(s) cells.
            Assert.That(cellTypes.n, Is.EqualTo(MapCellType.Trail), "N");
            Assert.That(cellTypes.ne, Is.EqualTo(MapCellType.Edge), "NE");
            Assert.That(cellTypes.w, Is.EqualTo(MapCellType.Trail), "W");
            Assert.That(cellTypes.center, Is.EqualTo(MapCellType.Trail), "Center");
            Assert.That(cellTypes.e, Is.EqualTo(MapCellType.Trail), "E");
            Assert.That(cellTypes.s, Is.EqualTo(MapCellType.Wall), "S");
            Assert.That(cellTypes.se, Is.EqualTo(MapCellType.Wall), "SE");
