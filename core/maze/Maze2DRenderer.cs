...
namespace Nour.Play.Maze {
    ...
    public class Maze2DRenderer {
        ...
        public Map2D GetMap2D() {
            var map = CreateMapForMaze(_options);
            for (var i = 0; i < _maze.Cells.Count; i++) {
                var mazeCell = _maze.Cells[i];
                var mapping = new CellsMapping(map, mazeCell, _options);
                var (nw, n, ne, w, center, e, sw, s, se) = GetCellTypes(mazeCell);
                Console.WriteLine($"{i} ({mazeCell}): {nw} {n} {ne} {w} {center} {e} {sw} {s} {se}");
                // TODO: implement a more generic approach. use one algo to
                // TODO: paint the maze on the map, and another (e.g., 3x3
                // TODO: window) algo to paint the walls on the map.
                if (nw != null) mapping.NWCells.ForEach(c => c.Tags.Add(nw));
                if (ne != null) mapping.NECells.ForEach(c => c.Tags.Add(ne));
                if (sw != null) mapping.SWCells.ForEach(c => c.Tags.Add(sw));
                if (se != null) mapping.SECells.ForEach(c => c.Tags.Add(se));
                if (w != null) mapping.WCells.ForEach(c => c.Tags.Add(w));
                if (e != null) mapping.ECells.ForEach(c => c.Tags.Add(e));
                if (n != null) mapping.NCells.ForEach(c => c.Tags.Add(n));
                if (s != null) mapping.SCells.ForEach(c => c.Tags.Add(s));
                if (center != null) mapping.CenterCells.ForEach(c => c.Tags.Add(center));
            }
            return map;
        }
        ...
        public (
            string nw, string n, string ne,
            string w, string center, string e,
            string sw, string s, string se)
        GetCellTypes(MazeCell mazeCell) {
            string nw, n, ne, w, center, e, sw, s, se;
            nw = n = ne = w = sw = se = null; // optionals
            center = mazeCell.Attributes.ContainsKey("MAP2D_CELL_TYPE") ?
                    mazeCell.Attributes["MAP2D_CELL_TYPE"] :
                mazeCell.IsVisited ? MapCellType.Trail :
                MapCellType.Wall;

            Func<MazeCell, Vector, (string, string, string)> oneSideTypes =
                (MazeCell cell, Vector side) => {
                    if (cell.Neighbors(side).HasValue) {
                        if (cell.Links(side).HasValue) {
                            return (null, MapCellType.Trail, null);
                        } else if (cell.IsVisited || cell.Neighbors(side).Value.IsVisited) {
                            return (null, MapCellType.Wall, null);
                        } else {
                            return (null, MapCellType.Void, null);
                        }
                    } else {
                        if (cell.IsVisited) {
                            return (MapCellType.Wall, MapCellType.Wall, MapCellType.Wall);
                        } else {
                            return (MapCellType.Void, MapCellType.Void, MapCellType.Void);
                        }
                    }
                };
            (nw, n, ne) = oneSideTypes(mazeCell, Vector.North2D);
            (ne, e, se) = oneSideTypes(mazeCell, Vector.East2D);
            (sw, s, se) = oneSideTypes(mazeCell, Vector.South2D);
            (nw, w, sw) = oneSideTypes(mazeCell, Vector.West2D);
            if (mazeCell.X == 3 && mazeCell.Y == 1) {
                Console.WriteLine($"{mazeCell}: {ne} {e} {se}");
            }

            // if (mazeCell.Neighbors(Vector.East2D).HasValue) {
            //     if (mazeCell.Links(Vector.East2D).HasValue) {
            //         e = MapCellType.Trail;
            //     } else if (mazeCell.IsVisited || mazeCell.Neighbors(Vector.East2D).Value.IsVisited) {
            //         e = MapCellType.Wall;
            //     } else {
            //         e = MapCellType.Void;
            //     }
            ...
            //         ne = e = se = MapCellType.Wall;
            //     } else {
            //         ne = e = se = MapCellType.Void;
            //     }
            // }
            // if (mazeCell.Neighbors(Vector.South2D).HasValue) {
            //     s = mazeCell.Links(Vector.South2D).HasValue ?
            //                         MapCellType.Trail :
            //                     mazeCell.IsVisited || (mazeCell.Neighbors(Vector.South2D).HasValue && mazeCell.Neighbors(Vector.South2D).Value.IsVisited) ?
            //                         MapCellType.Wall :
            //                         MapCellType.Void;
            // } else {
            //     sw = s = se = mazeCell.IsVisited ?
            //         MapCellType.Wall :
            //         MapCellType.Void;
            // }
            // if (mazeCell.Neighbors(Vector.West2D).HasValue) {
            //     w = mazeCell.Links(Vector.West2D).HasValue ?
            //             MapCellType.Trail :
            //         mazeCell.IsVisited || mazeCell.Neighbors(Vector.West2D).Value.IsVisited ?
            //             MapCellType.Wall :
            //             MapCellType.Void;
            // } else {
            //     nw = w = sw = mazeCell.IsVisited ?
            //         MapCellType.Wall :
            //         MapCellType.Void;
            // }
            // if (mazeCell.Neighbors(Vector.North2D).HasValue) {
            //     n = mazeCell.Links(Vector.North2D).HasValue ?
            //             MapCellType.Trail :
            //         mazeCell.IsVisited || mazeCell.Neighbors(Vector.North2D).Value.IsVisited ?
            //             MapCellType.Wall :
            //             MapCellType.Void;
            // } else {
            //     nw = n = ne = mazeCell.IsVisited ?
            //         MapCellType.Wall :
            //         MapCellType.Void;
            // }
            if (mazeCell.Neighbors(Vector.East2D).HasValue &&
                mazeCell.Neighbors(Vector.North2D).HasValue) {
                var diagonalCell = mazeCell.Neighbors(Vector.East2D).Value.Neighbors(Vector.North2D);
                ne = MapCellType.Wall;
                if (mazeCell.Links(Vector.East2D).HasValue &&
                    mazeCell.Links(Vector.North2D).HasValue &&
                    mazeCell.Links(Vector.East2D).Value.Links(Vector.North2D).HasValue &&
                    mazeCell.Links(Vector.North2D).Value.Links(Vector.East2D).HasValue) {
                    ne = MapCellType.Trail;
                } else if (new bool[] {
                        mazeCell.Links(Vector.East2D).HasValue,
                        mazeCell.Links(Vector.East2D).HasValue && mazeCell.Links(Vector.East2D).Value.Links(Vector.North2D).HasValue,
                        mazeCell.Links(Vector.North2D).HasValue,
                        mazeCell.Links(Vector.North2D).HasValue && mazeCell.Links(Vector.North2D).Value.Links(Vector.East2D).HasValue,
                    }.Count(b => b) >= 2) {
                    ne = MapCellType.Edge;
                } else if (mazeCell.Neighbors(Vector.East2D).Value.Links(Vector.North2D).HasValue &&
                            mazeCell.Neighbors(Vector.North2D).Value.Links(Vector.East2D).HasValue) {
                    ne = MapCellType.Edge;
                } else if (!mazeCell.Neighbors(Vector.East2D).Value.IsVisited &&
                            !mazeCell.Neighbors(Vector.North2D).Value.IsVisited &&
                            !mazeCell.Neighbors(Vector.East2D).Value.Neighbors(Vector.North2D).Value.IsVisited) {
                    ne = mazeCell.IsVisited ? MapCellType.Wall : MapCellType.Void;
                }
                if (ne == MapCellType.Edge) {
                    if (!mazeCell.IsVisited) {
                        center = MapCellType.Wall;
                    }
                    if (mazeCell.Neighbors(Vector.East2D).HasValue &&
                        !mazeCell.Neighbors(Vector.East2D).Value.IsVisited) {
                        mazeCell.Neighbors(Vector.East2D).Value.Attributes.Set("MAP2D_CELL_TYPE", MapCellType.Wall);
                    }
                    if (mazeCell.Neighbors(Vector.North2D).HasValue &&
                        !mazeCell.Neighbors(Vector.North2D).Value.IsVisited) {
                        mazeCell.Neighbors(Vector.North2D).Value.Attributes.Set("MAP2D_CELL_TYPE", MapCellType.Wall);
                    }
                    if (mazeCell.Neighbors(Vector.North2D).HasValue &&
                        mazeCell.Neighbors(Vector.North2D).Value.Neighbors(Vector.East2D).HasValue &&
                        !mazeCell.Neighbors(Vector.North2D).Value.Neighbors(Vector.East2D).Value.IsVisited) {
                        mazeCell.Neighbors(Vector.North2D).Value.Neighbors(Vector.East2D).Value.Attributes.Set("MAP2D_CELL_TYPE", MapCellType.Wall);
                    }
                }
            }
